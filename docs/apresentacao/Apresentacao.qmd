...
2. **Remover Background:** Vamos remover o fundo das imagens para facilitar na similaridade das imagens.
...
4. **Salvar Embeddings:** Vamos salvar os embeddings em um banco de dados para embeddings (FAISS) para facilitar o uso posterior.

. . .

5. **Buscar Imagens:** Vamos buscar imagens similares usando a similaridade do cosseno.
...
Função para ler imagens de um diretório e retornar em outro diretório pré-determinado as imagens
sem background, mas também a função retorna uma lista de imagens com um índice e o caminho original da imagem:
    ...
    for raiz, _, arquivos in os.walk(caminho_pasta_entrada):
        ...
        for arquivo in arquivos:
            if arquivo.lower().endswith(('.png', '.jpg', '.jpeg', '.webp', '.bmp', '.tiff')):
                ...
                extensao_saida = '.png' if extensao.lower() != '.png'
                  else extensao
                ...
                try:
                    ...
                    img_redimensionada.save(buffer_img, format='PNG' if extensao_saida == '.png'
                      else img_original.format)
                    buffer_img.seek(0)
                    ...
                    with open(caminho_completo_saida, 'wb') as o:
                        o.write(bytes_saida)
                    print(f"Processado e salvo: {caminho_completo_saida}
                      (Original: {caminho_completo_entrada})")
...
```{r, echo=TRUE, eval=FALSE}
# Uma pasta de exemplo com algumas imagens e subpastas
# Ex:
# minha_pasta/
# ├── imagem1.jpg
# ├── subpasta1/
...
pasta_de_imagens = 'database/natural_images_without_bg'
todas_as_imagens = ler_imagens_pillow(pasta_de_imagens)
print(f"\nTotal de imagens encontradas: {len(todas_as_imagens)}")

# Você pode acessar as imagens lidas, por exemplo:
# if todas_as_imagens:
#     todas_as_imagens[0].show() # Mostra a primeira imagem
```
...
#faiss.write_index(index, 'faiss_index_SF.index')  # Salva o índice em um arquivo
...
outputs = model(**inputs)
nova_embedding = outputs.logits.detach().numpy()
# Normalize os embeddings (para usar distância do cosseno)
...
# Indices das Imagens Retornadas
imagens_retornadas
imagens_retornadas = [x - 1 for x in imagens_retornadas]
